...
// ✅ Normaliza cualquier valor raro que venga del backend/panel a una categoría oficial
function normalizeCategoria(raw?: string): Category {
  if (!raw) return "OTROS";

  const c = raw.trim().toUpperCase();

  // aliases comunes
  if (c === "HOTDOG" || c === "HOTDOGS" || c === "HOT DOG" || c === "HOT DOGS")
    return "HOT DOG / PEPITO";
  if (c === "PEPITO" || c === "PEPITOS") return "HOT DOG / PEPITO";

  // match directo
  if ((CATEGORY_OPTIONS as readonly string[]).includes(c)) return c as Category;

  // si viene algo distinto -> OTROS
  return "OTROS";
}
...
export default function Home() {
  ...
  const [newCategoria, setNewCategoria] = useState<Category>("OTROS");

  // EDIT state
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editNombre, setEditNombre] = useState("");
  const [editPrecio, setEditPrecio] = useState("");
  const [editCategoria, setEditCategoria] = useState<Category>("OTROS");
    ...
    try {
      const res = await fetch(`${baseUrl}/menu`, {
        ...
        body: JSON.stringify({
          nombre,
          precio: precioNum,
          categoria: newCategoria, // ✅ siempre oficial
        }),
      });
  ...
  function startEdit(item: MenuItem) {
    ...
    setEditCategoria(normalizeCategoria(item.categoria));
  }
    ...
    try {
      const res = await fetch(`${baseUrl}/menu/${id}`, {
        ...
        body: JSON.stringify({
          nombre,
          precio: precioNum,
          categoria: editCategoria, // ✅ siempre oficial
        }),
      });
    ...
    <main style={{ padding: 24, fontFamily: "sans-serif" }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {/* ✅ SELECT de categorías oficiales (MAYÚSCULA) */}
          <select
            value={newCategoria}
            onChange={(e) => setNewCategoria(e.target.value as Category)}
            style={{
              padding: 10,
              width: "100%",
              maxWidth: 220,
          ...
          >
            ...
            <tbody>
              {pagedItems.map((item) => {
                ...
                const shownCategoria = normalizeCategoria(item.categoria);
                    ...
                    <td style={{ borderBottom: "1px solid #eee", padding: 8 }}>
                      {isEditing ? (
                        <select
                          value={editCategoria}
                          onChange={(e) =>
                            setEditCategoria(e.target.value as Category)
                          }
                          style={{
                            padding: 8,
                            width: 220,
