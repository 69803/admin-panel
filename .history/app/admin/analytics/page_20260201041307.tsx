...
function buildBuckets(rangeFrom: Date, rangeTo: Date, g: Granularity): Date[] {
  ...
  for (let safety = 0; safety < 5000; safety++) {
    ...
    if (cur.getTime() > end.getTime() && buckets[buckets.length - 1].getTime() !== end.getTime()) {
      // ensure last bucket start exists
      // (handled by break above, but keep safe)
    }
  }
  return buckets;
}
    ...
    >
      <div style={{ padding: 14, display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ fontWeight: 900, fontSize: 14, opacity: 0.92 }}>
          {metric === "monto" ? "Monto (ingresos)" : metric === "items" ? "Items vendidos" : metric === "pedidos" ? "Pedidos" : "Ticket promedio"}
        </div>
...
export default function AnalyticsPage() {
  ...
  const points = useMemo<Point[]>(() => {
    ...
    for (const p of hist) {
      ...
      // normalize key to existing bucket keys (we used b.toISOString of bucket starts)
      // bucketKey returns ISO of start. That should match.
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={{ ...card, marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => setPreset("diario")} style={chip(granularity === "diario")}>Diario</button>
            <button onClick={() => setPreset("semanal")} style={chip(granularity === "semanal")}>Semanal</button>
            <button onClick={() => setPreset("mensual")} style={chip(granularity === "mensual")}>Mensual</button>
            <button onClick={() => setPreset("anual")} style={chip(granularity === "anual")}>Anual</button>
          </div>
      ...
      {loading ? (
        ...
        <StockChart points={points} metric={metric} />
      )}
